...
var stepcarousel={
	...
	alignpanels:function(b$, config){
		var paneloffset=0
		config.paneloffsets=[paneloffset] //array to store upper left offset of each panel (1st element=0)
		config.panelwidths=[] //array to store widths of each panel
		config.$panels.each(function(index){ //loop through panels
			var $currentpanel=b$(this)
		...
		if (config.defaultbuttons.enable==true){ //if enable default back/forth nav buttons
			...
			b$(window).bind("load resize", function(){ //refresh position of nav buttons when page loads/resizes, in case offsets weren't available document.oncontentload
	...
	setlinkclass:function(pindex){
		jx("#board_carusel_nav a").removeClass("selected");
		jx("#board_carusel_nav #board_carusel_nav_"+(pindex+1)+" a").addClass("selected");
	},
    ...
    jx=jQuery.noConflict()
		jQuery(document).ready(function(jx){
			config.$gallery=jx('#'+config.galleryid)
			...
			config.onslideaction=(typeof config.onslide=="undefined")? function(){} : function(beltobj){jx(beltobj).stop(); config.onslide()} //attach custom "onslide" event handler
			config.oninit=(typeof config.oninit=="undefined")? function(){} : config.oninit //attach custom "oninit" event handler
			config.beltoffset=stepcarousel.getCSSValue(config.$belt.css('marginLeft')) //Find length of Belt DIV's left margin
			config.statusvars=config.statusvars || []  //get variable names that will hold "start", "end", and "total" slides info
			config.$statusobjs=[jx('#'+config.statusvars[0]), jx('#'+config.statusvars[1]), jx('#'+config.statusvars[2])]
			config.currentpanel=0
			stepcarousel.configholder[config.galleryid]=config //store config parameter as a variable
			if (config.contenttype[0]=="ajax" && typeof config.contenttype[1]!="undefined") //fetch ajax content?
				stepcarousel.getremotepanels(jx, config)
			else
				stepcarousel.alignpanels(jx, config) //align panels and initialize gallery
		}) //end document.ready
